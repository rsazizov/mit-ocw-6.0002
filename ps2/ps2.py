...
def get_best_path(digraph, start, end, path,
  ...
  # won't happen unless path is the most optimal so far
  if start == end:
    best_path = path
  else:
    ...
    for edge in edges:
      ...
      new_path = get_best_path(digraph, dest_name, end, new_path,
          max_dist_outdoors, best_dist, best_path)

      if best_path is None or new_path[1] < best_path[1]:
        best_path = new_path
        best_dist = best_path[1] if best_path is not None else best_dist
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
  ...
  """
  best_path = get_best_path(digraph, start, end, [[start], 0, 0],
